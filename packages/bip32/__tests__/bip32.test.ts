...
describe('BIP32', () => {
  test('fromSeed accepts a configurable master secret', () => {
    const seed = hexToBytes('000102030405060708090a0b0c0d0e0f');

    const implicitDefault = Bip32Node.fromSeed({ seed });
    const explicitDefault = Bip32Node.fromSeed({ seed, network: 'mainnet', masterSecret: 'Bitcoin seed' });
    const customSecret = Bip32Node.fromSeed({ seed, network: 'mainnet', masterSecret: 'Custom seed domain' });

    expect(implicitDefault.toXprv()).toBe(explicitDefault.toXprv());
    expect(customSecret.toXprv()).not.toBe(implicitDefault.toXprv());
  });

  test('hardened index helpers convert indices safely', () => {
    const hardenedZero = toHardenedIndex(0);

    expect(hardenedZero).toBe(0x80000000);
    expect(isHardenedIndex(hardenedZero)).toBe(true);
    expect(fromHardenedIndex(hardenedZero)).toBe(0);

    expect(() => toHardenedIndex(0x80000000)).toThrow();
    expect(() => fromHardenedIndex(1)).toThrow();
  });

  test('N(CKDpriv) equals CKDpub(N(...)) for non-hardened derivation', () => {
    const master = Bip32Node.fromSeed({ seed: hexToBytes('000102030405060708090a0b0c0d0e0f') });
    const privateParent = master.derivePath("m/0'");

    const fromPrivate = privateParent.derivePath('1').neuter().toXpub();
    const fromPublic = privateParent.neuter().derivePath('1').toXpub();

    expect(fromPrivate).toBe(fromPublic);
  });

  test.each(VECTORS)('matches official vectors for seed $seedHex', (vector) => {
    const master = Bip32Node.fromSeed({ seed: hexToBytes(vector.seedHex) });

    for (const pathVector of vector.paths) {
      const node = master.derivePath(pathVector.path);

      expect(node.toXpub()).toBe(pathVector.xpub);
      expect(node.toXprv()).toBe(pathVector.xprv);

      const parsedPrv = Bip32Node.fromExtendedKey(pathVector.xprv);
      const parsedPub = Bip32Node.fromExtendedKey(pathVector.xpub);

      expect(parsedPrv.toXprv()).toBe(pathVector.xprv);
      expect(parsedPrv.toXpub()).toBe(pathVector.xpub);
      expect(parsedPub.toXpub()).toBe(pathVector.xpub);
    }
  });

  test('rejects invalid extended key vectors', () => {
    for (const key of INVALID_EXTENDED_KEYS) {
      expect(() => Bip32Node.fromExtendedKey(key)).toThrow();
    }
  });
});
